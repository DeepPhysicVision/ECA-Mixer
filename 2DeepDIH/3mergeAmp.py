...
for i in range(3):
    img = Image.open(f'./{result_dir}/Amplitude/Best_{i}.bmp')   # dl
    img = pil2tensor(img)
...
imageio.imwrite(f'./{result_dir}/Dih_Amp.bmp', (recon_srgb * np.iinfo(np.uint8).max).round().astype(np.uint8))  #dl
