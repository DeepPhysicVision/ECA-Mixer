...
##################################################################### dl
target_dir = 'QiuYin'     #### change 1
target_amp = Image.open(f'./Color/{target_dir}/A.bmp')#.convert("RGB")
target_amp = pil2tensor(target_amp)
target_amp = target_amp.view(target_amp.shape[-3],target_amp.shape[-2],target_amp.shape[-1])

result_dir = f'Color/{target_dir}/AttentionNet'     #### change 2 AttentionNet  EcaNet  ConvNet
recon_amp = []

for i in range(3):
    img = Image.open(f'./{result_dir}/Amp/Best_{i}.bmp')
    img = pil2tensor(img)
...
imageio.imwrite(f'./{result_dir}/Dl_Amp.bmp', (recon_srgb * np.iinfo(np.uint8).max).round().astype(np.uint8))
...
'''
# just for the simulated situation
# calculate metrics
...
recon_amp = []
for i in range(3):
    img = Image.open(f'./{result_dir}/Amp/Back_{i}.bmp')
    img = pil2tensor(img)
    img = img.view(img.shape[-2], img.shape[-1])
...
# save reconstructed image in srgb domain
recon_srgb = srgb_lin2gamma(np.clip(recon_amp ** 2, 0.0, 1.0))
imageio.imwrite(f'./{result_dir}/Back_Amp.bmp', (recon_srgb * np.iinfo(np.uint8).max).round().astype(np.uint8))
