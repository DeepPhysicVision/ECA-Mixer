...
# z, wave and deltaX are the key parameters, you should change it based on your our data setting
def main(z=0.85, deltaX=1.85e-3):
    ...
    src_save = f'./data/{dir}/AttentionNet'
    ...
    for j in range(3):
        ...
        temp = torch.cat([tensor_diff, tensor_diff],1)
        temp = torch.fft.fft2(temp)
    ...
    LossBack = torch.mean(torch.abs(torch.abs(BackTarget)*torch.abs(BackTarget) - target*target)) / 2
    best = LossBack / 5 # early stop setting, which can be changed based on the reconstruction results.
    ...
    print('Training time', TrainTime) # the training time may differ with different GPU and environment
    temp = np.zeros((3,3))
